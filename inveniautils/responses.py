...
def split_multipart(response):
    ...
    if hasattr(response, "headers") and "content-type" in response.headers:
        ...
        content_type_data = [
            param.strip() for param in response.headers["content-type"].split(";")
        ]
    ...
    content_type_parameters = dict(
        re.search(r'(.*?)="(.*)"', param).groups() for param in content_type_data[1:]
    )
    ...
    parts = re.split(
        r"\r\n--" + re.escape(content_type_parameters["boundary"]) + r"(?:--)?\r\n",
        toParse,
    )
