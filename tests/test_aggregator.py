...
from inveniautils.aggregator import (
    TimeSeriesAggregate, extend_date_range, group_release_dates
)
from inveniautils.dates import localize, utc
from inveniautils.datetime_range import (
    Bound, DatetimeRange, period_ending_as_range,
)
...
class TestTimeSeriesAggregate(unittest.TestCase):
    def test_hourly(self):
        ...
        expected = [
            {
                'dtr': exp_dtr[4], 'rd': dates[1], 'n': 'a',
                'v': [0.011], 'tag': None,
            },
            {
                'dtr': exp_dtr[4], 'rd': dates[2], 'n': 'a',
                'v': [0.011, 0.121], 'tag': None,
            },
            {
                'dtr': exp_dtr[4], 'rd': dates[3], 'n': 'a',
                'v': [0.031, 0.131], 'tag': None,
            },
            {
                'dtr': exp_dtr[4], 'rd': dates[1], 'n': 'b',
                'v': [0.012], 'tag': None,
            },
            {
                'dtr': exp_dtr[4], 'rd': dates[2], 'n': 'b',
                'v': [0.022, 0.122], 'tag': None,
            },
            {
                'dtr': exp_dtr[5], 'rd': dates[3], 'n': 'a',
                'v': [0.231], 'tag': None,
            },
        ]
    ...
    def test_relevancy_check(self):
        ...
        rd = [
            datetime(2015, 3, 3, 5, 10, 6),
            datetime(2015, 3, 3, 5, 15),
        ]
        dtrs = [DatetimeRange.containing(dt) for dt in dates]
        ...
        expected = [
            {
                'dtr': exp_dtrs[2], 'n': 'ERCO',
                'rd': rd[0], 'lmp': [21.49], 'tag': None
            },
            {
                'dtr': exp_dtrs[2], 'n': 'ERCO',
                'rd': rd[1], 'lmp': [21.49, 22.77], 'tag': None
            },
            {
                'dtr': exp_dtrs[2], 'n': 'FPLL',
                'rd': rd[0], 'lmp': [22.47], 'tag': None
            },
            {
                'dtr': exp_dtrs[2], 'n': 'FPLL',
                'rd': rd[1], 'lmp': [22.47, 23.89], 'tag': None
            },
        ]
    ...
    def test_reducer_complicated(self):
        ...
        test = [
            ...
            {
                'dt': DatetimeRange.containing(
                    localize(datetime(2016, 5, 1, 5), utc)),
                'n': 1348265112,
...
class TestExtendDateRange(unittest.TestCase):
    def test_hourly_basic(self):
        ...
        self.assertEqual(
            extend_date_range(test, timedelta(hours=1)),
            expected,
        )
